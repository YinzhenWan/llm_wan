...
class LoraArguments:
    ...
    lora_target_modules: List[str] = ["q", "k", "v", "o", "o_proj", "ffn1_proj", "ffn2_proj"]
...
def preprocess(
    ...
    roles = {"user": "[BOS]user", "assistant": "[BOS]assistant"}  # 角色扮演说明

    start_id = tokenizer.bos_token_id  # 开始token_id
    end_id = tokenizer.eos_token_id  # 结束token_id
    nl_tokens = tokenizer('\n').input_ids  # 获取\n的token_id
    ...
    for i, source in tqdm(enumerate(sources)):
        ...
        input_id, target = [], []
        system = [start_id] + _system + tokenizer(system_message).input_ids + [
            end_id] + nl_tokens  # 获取系统提示的id: start_id+system对应id+'\n'id+‘你是一个人工智能’id+end_id+\n id
        input_id += system
        target += [start_id] + [IGNORE_TOKEN_ID] * (len(system) - 3) + [
            end_id] + nl_tokens  # 获取初始标签Id: start_id+[-100]*(len(system)-3)+end_id+\n id 这里的3指的是 [start_id]+[end_id]+nl_tokens的列表长度
...
def main():
    ...
    data_module = make_supervised_data_module(
        tokenizer=tokenizer, data_path=args.sft_data_path, eval_data_path='', max_len=args.max_len,
        cache_file=args.cache_data_path
    )
