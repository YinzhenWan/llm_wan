...
from model.wan_config import WANConfig
...
class WANRMSNorm(nn.Module):  ##不使用flash-attn的RMSNorm
...
class WANFlash_attnRMSNorm(nn.Module):  # 使用flash-attn的RMSNorm归一化方法
...
####参考Llama2源码修改的ROPE代码#########
class WANRotryEmbedding(nn.Module):
...
class WANNTKRotaryEmbedding(WANRotryEmbedding):
...
class WANFFN(nn.Module):  # transformer block继承attention block之后的前馈层
...
class WANNormAttention(
    ...
    def get_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = WANRotryEmbedding(dim=self.head_dim, max_position_embeddings=self.max_position_embeddings,
                                                base=self.rope_theta)
        elif self.config.rope_scaling == 'NTK':
            # print('当前正在使用NTK进行缩放')
            self.rotary_emb = WANNTKRotaryEmbedding(dim=self.head_dim,
...
class WANSdpaAttention(WANNormAttention):
    ...
    def forward(
        ...
        if output_attentions:
            logger.warning_once(
                "WANModel 正在使用 WANSdpaAttention, 但是 `torch.nn.functional.scaled_dot_product_attention` 不支持`output_attentions=True`. 需要重新手动计算attention_out  "
            )
...
WAN_ATTENTION_CLASSES = {
    "eager": WANNormAttention,  # 不使用torch中的flash
    # "flash": WANFlashAttention2, #使用flash_attn
    "sdpa": WANSdpaAttention,  # 使用torch中的flash
}


class WANDecoderlayer(nn.Module):
    def __init__(self, config: wanconfig, layer_idx: int, select_rmsnorm: 'WANRMSNorm'):
        super().__init__()
        self.hidden_size = config.hidden_size
        config.attn_implementation = 'sdpa'
        # print(config)
        self.self_attn = WAN_ATTENTION_CLASSES[config.attn_implementation](config=config, layer_idx=layer_idx)

        # print(NDL_ATTENTION_CLASSES[config.attn_implementation](config=config, layer_idx=layer_idx))
        self.mlp = WANFFN(config)  # 前馈神经网络层
        self.select_rmsnorm = select_rmsnorm  # 选择RMSNorm层是否使用flash_attn
        if 'Flash' not in self.select_rmsnorm:
            self.input_layernorm = WANRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_attention_layernorm = WANRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        else:
            self.input_layernorm = WANFlash_attnRMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # 输入层的RMSNorm
            self.post_attention_layernorm = WANFlash_attnRMSNorm(config.hidden_size,
...
class WANPreTrainedModel(PreTrainedModel):
    ...
    base_model_prefix = "WANmodel"
    supports_gradient_checkpointing = True
    _no_split_modules = ["WANDecoderlayer"]
...
class WANModel(WANPreTrainedModel):
    def __init__(self, config: wanconfig):
        ...
        if config.use_flash_attn:
            try:
                ...
                select_norm = 'WANFlash_attnRMSNorm'
                self.norm = WANFlash_attnRMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # RMSNorm层
            except:
                select_norm = 'WANRMSNorm'
                self.norm = WANRMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # RMSNorm层
        self.layers = nn.ModuleList(
            [WANDecoderlayer(config, layer_idx, select_norm) for layer_idx in range(config.num_hidden_layers)]
        )  # 32个decoder层
...
class WANForCausalLM(WANPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = WANModel(config).bfloat16()
...
if __name__ == '__main__':
    # 实例化model
    wanconfig = WANConfig()
    model = WANForCausalLM(wanconfig)
    print(model)
