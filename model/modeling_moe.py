...
class MOERMSNorm(nn.Module):  ##不使用flash-attn的RMSNorm----与非MOE架构相同，这里复制原有的RMSNorm即可
...
class MOEFlash_attnRMSNorm(nn.Module):  # 使用flash-attn的RMSNorm归一化方法
...
####参考Llama2源码修改的ROPE代码#########
class MOERotryEmbedding(nn.Module):  # 与非MOE架构的ROPE旋转位置编码相同，这里直接复制即可
    ...
    def forward(self, x, position_ids):  # x:[batch_size,num_attention_heads, seq_len, head_size]
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            freqs = (inv_freq_expanded.float() @ position_ids_expanded.float()).transpose(1,
                                                                                          2)  # inv_freq与position_ids相乘，并转置，交换第1维与第2维
...
class MOENTKRotaryEmbedding(MOERotryEmbedding):
...
class MOEFFN(nn.Module):  # transformer block继承attention block之后的前馈层
    def __init__(self, config=moeconfig, hidden_size=None, intermediate_size=None):
        super().__init__()
...
class MoEGate(nn.Module):  # MOE结构中的门控单元
        ...
        if self.training and self.alpha > 0.0:  # 如果是预训练阶段，并且专家平衡因子大于0
            ...
            if self.seq_aux:  # 如果是seq层面的均衡损失
                ...
                mask_ce = F.one_hot(topk_idx_for_aux_loss.view(-1),
                                    num_classes=self.n_routed_experts)  # 将选择的专家id转换为独热编码
...
class MoE(nn.Module):  ## MOE层
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.num_experts_active_topk = config.num_experts_per_tok
        self.experts = nn.ModuleList(
            [MOEFFN(config, intermediate_size=config.moe_intermediate_size) for i in range(config.n_routed_experts)])
        self.gate = MoEGate(config)
        if config.n_shared_experts is not None:  # 如果设置共享专家
            intermediate_size = config.moe_intermediate_size * config.n_shared_experts
            self.shared_experts = MOEFFN(config=config, intermediate_size=intermediate_size)  # 共享专家
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states.view(-1, hidden_states.shape[
            -1])  # 将hidden_states形状更改为二维张量，第二维度是hidden_dim，第一维度是bsz*seq_len
        flat_topk_idx = topk_idx.view(
            -1)  # topk_idx:bsz*seq_len,num_experts_active_topk ===>bsz*seq_len*num_experts_active_topk,展平操作
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    return hidden_states.reshape(batch, num_key_value_heads * n_rep, seqlen,
                                 head_dim)  # 最后将张量reshape为batch, num_key_value_heads * n_rep, seqlen, head_dim


## MOE架构的attention与非MOE模型的attention代码相同，这里不用修改
class MOENormAttention(
    nn.Module):  # Transformer Block的自注意力机制-----GQA分组查询注意力机制---如果设置了num_key_value_heads大于1会使用GQA机制，否则就是全局注意力机制
    def __init__(self, config: moeconfig, layer_idx):
        super().__init__()
        self.config = config  # 模型的配置文件
        ...
        self.q = nn.Linear(self.hidden_size, self.num_heads * self.head_dim,
                           bias=config.attention_bias)  # 输入维度hidden_size,输出维度num_head(头数)*head_dim(每个头的维度)，相当于多头输出的cat
    ...
    def get_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = MOERotryEmbedding(dim=self.head_dim,
                                                   max_position_embeddings=self.max_position_embeddings,
                                                   base=self.rope_theta)
        elif self.config.rope_scaling == 'NTK':
            # print('当前正在使用NTK进行缩放')
            self.rotary_emb = MOENTKRotaryEmbedding(dim=self.head_dim,
                                                       max_position_embeddings=self.max_position_embeddings,
                                                       base=self.rope_theta)
        else:
    ...
    def forward(
        ...
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(
            self.head_dim)  # softmax的输入 [batch_size,self.num_heads,seq_len,seq_len]
        ...
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(
            query_states.dtype)  # 计算softmax(Q*KT)
        ...
        attn_output = attn_output.transpose(1,
                                            2).contiguous()  # 转置并连续化 [batch_size,seq_len,self.num_heads,self.head_dim]
        attn_output = attn_output.reshape(batch_size, seq_len,
                                          self.hidden_size)  # reshape为【batch_size, seq_len, self.hidden_size】
...
class MOESdpaAttention(MOENormAttention):  ## MOE架构的attention与非MOE模型的attention代码相同，这里不用修改
    ...
    def forward(
        ...
        if output_attentions:
            logger.warning_once(
                "MOEModel 正在使用 MOESdpaAttention, 但是 `torch.nn.functional.scaled_dot_product_attention` 不支持`output_attentions=True`. 需要重新手动计算attention_out  "
            )
...
MOE_ATTENTION_CLASSES = {
    "eager": MOENormAttention,  # 不使用torch中的flash
    # "flash": MOEFlashAttention2, #使用flash_attn
    "sdpa": MOESdpaAttention,  # 使用torch中的flash
}


### 开始构造MOE模型的decoder层
class MOEDecoderlayer(nn.Module):
    def __init__(self, config: moeconfig, layer_idx: int, select_rmsnorm: 'MOERMSNorm'):
        super().__init__()
        self.hidden_size = config.hidden_size
        config.attn_implementation = 'sdpa'
        # print(config)
        self.self_attn = MOE_ATTENTION_CLASSES[config.attn_implementation](config=config, layer_idx=layer_idx)

        # print(MOE_ATTENTION_CLASSES[config.attn_implementation](config=config, layer_idx=layer_idx))
        # 选择mlp,如果设置专家总数以及layer_idx大于等于第一个稠密层替换索引以及layer_idx%moe_layer_freq==0则设置为MOE，否则设置为普通的FFN层
        self.mlp = MoE(config) if (config.n_routed_experts is not None and \
        ...
        if 'Flash' not in self.select_rmsnorm:
            ...
            self.input_layernorm = MOEFlash_attnRMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # 输入层的RMSNorm
            self.post_attention_layernorm = MOEFlash_attnRMSNorm(config.hidden_size,
                                                                    eps=config.rms_norm_eps)  # attention之后的RMSNorm
    ...
    def __init__(self, config: moeconfig):
        ...
        if config.use_flash_attn:
            try:
                ...
                select_norm = 'MOEFlash_attnRMSNorm'
                self.norm = MOEFlash_attnRMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # RMSNorm层
            except:
                select_norm = 'MOERMSNorm'
                self.norm = MOERMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # RMSNorm层
        else:
            select_norm = 'MOERMSNorm'
            self.norm = MOERMSNorm(config.hidden_size, eps=config.rms_norm_eps)  # RMSNorm层
        self.layers = nn.ModuleList(
            [MOEDecoderlayer(config, layer_idx, select_norm) for layer_idx in range(config.num_hidden_layers)]
        )  # 32个decoder层
        # self.norm = MOERMSNorm(config.hidden_size, eps=config.rms_norm_eps)#RMSNorm层
...
class MOEForCausalLM(MOEPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = MOEModel(config).bfloat16()
...
if __name__ == '__main__':
    ...
    print(
        f"当前MOE模型激活专家层的激活参数大约为：{model_size_act_experts / 10000 ** 2 / 10 / (moeconfig.n_routed_experts // moeconfig.num_experts_per_tok):.1f}B parameters")
