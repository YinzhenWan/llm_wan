...
from tokenizers.pre_tokenizers import Punctuation, Digits, \
    Metaspace  # 导入预分词处理器，如Punctuation用于单独处理标点符号，Digits用于数字处理，Metaspace用于特殊字符（如空格）的处理
from tokenizers.normalizers import NFKC  # 导入NFKC正则化器，这是一种Unicode标准化形式，用于文本规范化，减少变体和不一致性
from transformers import \
    PreTrainedTokenizerFast  # 导入Hugging Face transformers库中的PreTrainedTokenizerFast类，这是一个预训练好的快速tokenizer接口，常用于与预训练模型配套使用
...
def train_tokenizer(cropus_file: str, max_train_line: int = None, vocab_size: int = 40960,
