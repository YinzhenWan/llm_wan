...
tokenizer = AutoTokenizer.from_pretrained('/root/autodl-tmp/tokenizer', trust_remote_code=True)
...
def split_txt_cropus_to_chunk_data(
    ...
    for i, line in enumerate(texts):
        ...
        if buffer_len >= batch_size or i == len(
                texts) - 1:  # 当累计长度达到batch_size时，或者进入最后一个循环时，对累计的数据进行划分，保证最长的seq_len为512
