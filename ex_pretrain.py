...
config = WANConfig()
from model.modeling_wan import WANForCausalLM
...
# 如果是增量预训练，请在模型加载的时候加载之前训练好的模型,即NDLSLM_0.8B-base下载至路径/root/autodl-tmp/model_save/下面
model = WANForCausalLM(config).from_pretrained('/root/autodl-tmp/model_save/0_8B_base/Ndlcwx/NDLSLM_0___8B-base').to(
    device
) ##增量预训练
